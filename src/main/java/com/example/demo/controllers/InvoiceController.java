...
public class InvoiceController extends BaseErrorController {
    ...
    public String invoiceForm(
        ...
        if (id != null) {
            ...
            if (invoice == null) {
                return error(model, "Rechnung nicht gefunden.", null);
            }
        } else {
            invoice = new Invoice();
            if (bookingId != null && !bookingId.isEmpty()) {
                ...
                if (booking == null || booking.getID().isEmpty()) {
                    return error(model, "Buchung nicht gefunden oder ungültig.", null);
                }
    ...
    public String createInvoice(@RequestParam("bookingId") String bookingId, Model model) {
        Booking booking = bookingService.getById(bookingId);
        if (booking == null) {
            return error(model, "Buchung nicht gefunden.", null);
        }
    ...
    public String saveInvoice(@RequestParam("invoiceId") String invoiceId, Model model) {
        ...
        if (invoice == null) {
            return error(model, "Invoice not found.", null);
        }
